...
def symlink_embedded_document_to_workdir(
    doc: Document, paths: WorkerPaths
) -> ProcessedFile:
    ...
    raw_path = ds_file.locate(paths)
    with contextlib.suppress(FileExistsError):
        os.symlink(raw_path, symlink_path)
    return WorkerFile.from_parent(ds_file, symlink_path, paths)
...
class _PreprocessingFunction[S: ErrorSource, R](Protocol):
    def __call__(self, doc: S, *args, **kwargs) -> R: ...


def reports_errors[S: ErrorSource, R](
    errors: tuple[type[Exception]], exc_cls: type[ProcessingError[S]]
) -> Callable[
    [_PreprocessingFunction[S, R]], _PreprocessingFunction[S, R | ProcessingError[S]]
]:

    def parent_wrapper(
        f: _PreprocessingFunction[S, R],
    ) -> _PreprocessingFunction[S, R | ProcessingError[S]]:
        if inspect.iscoroutinefunction(f):

            @wraps(f)  # noqa: F821
            async def wrapper(doc: S, *args, **kwargs) -> R | ProcessingError[S]:
                try:
            ...
            @wraps(f)
            def wrapper(doc: ProcessedFile, *args, **kwargs) -> R | ProcessingError[S]:
