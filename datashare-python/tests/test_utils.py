...
def test_lease_resource_evicted_while_leased_defers_eviction_callback() -> None:
    # Given
    mock = MagicMock()
    eviction_callback = mock.evict
    shared = SharedResources(cache_size=1, eviction_callback=eviction_callback)
    # When
    with shared.lease_resource("k0", lambda: "leased"):
...
def test_lease_resource_eviction_callback_waits_for_last_lease() -> None:
    # Given
    mock = MagicMock()
    eviction_callback = mock.evict
    shared = SharedResources(cache_size=1, eviction_callback=eviction_callback)
    # When
    with shared.lease_resource("k0", lambda: "leased") as first:
        with shared.lease_resource("k0", lambda: "not_called") as second:
            shared.get_or_cache_resource("k1", lambda: "other")
        # Then
        eviction_callback.assert_not_called()
...
def test_lease_resource_released_resource_is_evicted_immediately() -> None:
    # Given
    mock = MagicMock()
    eviction_callback = mock.evict
    shared = SharedResources(cache_size=1, eviction_callback=eviction_callback)
...
def test_lease_resource_exit_defers_eviction_callback_of_leased_resource() -> None:
    # Given
    mock = MagicMock()
    eviction_callback = mock.evict
    shared = SharedResources(cache_size=1, eviction_callback=eviction_callback)
    lease = shared.lease_resource("k0", lambda: "leased")
    # When
    with shared:
        lease.__enter__()
    # Then
...
def test_lease_resource_is_not_closed_by_concurrent_eviction() -> None:
    # Given
    class Resource:
    ...
    def use_resource() -> None:
        with shared.lease_resource("k0", Resource) as resource:
            leased.set()
            evicted.wait(5)
    ...
    thread.start()
    leased.wait(5)
    shared.get_or_cache_resource("k1", Resource)
    evicted.set()
    thread.join()
    # Then
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            False,
            True,
            Path("workdir", TEST_PROJECT, "wf_type", "__cache__", "caching_key"),
        ),
        (
            True,
            False,
        ...
        (
            ...
            True,
            True,
            Path("workdir", TEST_PROJECT, "act_type", "__cache__", "caching_key"),
        ),
        (
            False,
            True,
